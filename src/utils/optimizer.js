...
// Optimizes code comments and spacing, keeping syntax intact
export const optimizeCodeBlocks = (text) => {
  if (!text) return '';
  
  const codeBlockRegex = /```([a-zA-Z0-9+\-#]*)\n([\s\S]*?)\n```/g;
  
  const minifyRawCode = (code) => {
    let result = code;
    // Remove Javascript/C++/Java single-line comments (except URLs)
    result = result.replace(/(^|[^\/:])\/\/.*$/gm, '$1');
    // Remove Python/Ruby/Bash single-line comments
    result = result.replace(/(^|[^"'\s])\s*#.*$/gm, '$1');
    // Remove C-style block comments
    result = result.replace(/\/\*[\s\S]*?\*\//g, '');
    // Remove HTML comments
    result = result.replace(/<!--[\s\S]*?-->/g, '');
    // Remove double lines and minimize consecutive newlines to at most 1
  ...
  if (codeBlockRegex.test(text)) {
    ...
    return text.replace(codeBlockRegex, (match, lang, code) => {
      return `\`\`\`${lang}\n${minifyRawCode(code).trim()}\n\`\`\`\n`;
    });
  } else {
    // Check if the entire block is likely a programming script
    const isLikelyCode = /def\s+\w+\(|function\s+\w+\(|import\s+\w+|#include|class\s+\w+|\{\s*|;\s*$/m.test(text);
    if (isLikelyCode) {
      return minifyRawCode(text);
    }
    return text;
  }
};
...
export const optimizeText = (text, options) => {
  ...
  // 1. Minify code blocks comments and spacing if enabled
  if (options.minifyCode) {
    tempText = optimizeCodeBlocks(tempText);
  }

  // 2. Shield markdown code blocks from stop-word and vowel stripping
  const codeBlockRegex = /(```[a-zA-Z0-9+\-#]*\n[\s\S]*?\n```)/g;
  let blockIndex = 0;
  tempText = tempText.replace(codeBlockRegex, (match) => {
    const placeholder = `[SHIELD_CODE_BLOCK_${blockIndex}]`;
    codeBlocksMap[placeholder] = match;
    blockIndex++;
    return placeholder;
  });

  let optimized = tempText;
  ...
  if (strength === 'standard') {
    ...
    if (strength === 'ultra') {
      ...
      const processedTokens = tokens.map(token => {
        ...
        // Skip placeholders
        if (token.startsWith('[SHIELD_CODE_BLOCK_')) return token;
  ...
  // 3. Rehydrate shielded code blocks
  Object.keys(codeBlocksMap).forEach(placeholder => {
    optimized = optimized.replace(placeholder, codeBlocksMap[placeholder]);
  });
